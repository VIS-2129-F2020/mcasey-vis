...
6. Average Household size of the tract (continous): avg_hh
...
mke_tracts <- get_decennial(geography = "tract", variables = vars, 
  ...
  mutate(maj_race = case_when(
    ...
    (white + black + native + asian + asian + pac_isl + other + multi) / tot_pop < 0.5 ~ "other",
    TRUE ~ "None")) %>%
...
ggplot(mke_tracts,
  ...
  scale_color_discrete(name = "Majority Household Type",
                       labels = c("Family",
                                  "Non Family")) +
  theme_gray()
```
...
ggplot(mke_tracts, 
  ...
  scale_fill_discrete(name = "Majority Household Type",
                      labels = c("Family",
                                 "Non Family")) +
  theme_linedraw()
```
